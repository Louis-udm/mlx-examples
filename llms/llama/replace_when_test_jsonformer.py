...
class Jsonformer:
    ...
    def generate_string(self) -> str:
        ...
        input_tokens = mx.array(input_tokens.tolist())

        responser = self.model.generate(
            input_tokens,
        ...
        response = []
        skip=0
        for token in responser:
            response.append(token)
            if [t[0] for t in response[-5:]] == [13]*5: # 5 newlines
                break
            elif len(response) >= 500:
                break
            elif (len(response) % 5) == 0:
                # It is perfectly ok to eval things we have already eval-ed.
                s = self.tokenizer.decode([t.item() for t in response])
        ...
        response = self.tokenizer.decode([t.item() for t in response])
